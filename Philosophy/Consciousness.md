...
Like physics, chemistry also has deceptively taught us that what may seem a single blazing "lifeforce" in nature — take a fire for example — may not really be a living creature of one nature and quality. A fire, specifically, is often decomposable into the bonding re-configurations of hydrogen and carbon atoms in hydrocarbons with oxygen atoms, wherein light is emitted from the accelerated/re-configured electrons, and the all-around re-configurations are energetic and fuel further such hydrocarbon-splitting combustion reactions. The quantum states of the electrons are divisible and physically exclude one another. The light emitted is made of photons, which are bosons, but their wave functions nevertheless can vary between each other. Similarly, divided units are attributed as the sole mechanics of neurology as well, but fires don’t experience human-like consciousness, where a singularization happens in which multiple phenomena, experienced at once, coalesce to one observer that *is* the fire (to our knowledge). Our neurons that produce the individual observer — somehow do — they singularize, with messengers that unify not just information communication, but the "us" that observes those phenomena.
