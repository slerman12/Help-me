...
In order to define a number line, we can just define $\mathbb{R}$ and the relationships $<, >$. But in order to make that literally true, we hav to assume both the literal truth of those numbers and the literal truth of those relationships as axioms.
