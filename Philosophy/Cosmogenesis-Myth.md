...
Another critique, I dare add, is that the cheap hack of time into equivalence has never been used before in defining equivalents, but mathematically speaking, this is internally consistent with the definition of deduction as having mandatory intermediary deductive steps, something that was always universally true. This allows equivalents to be distinguished and not swappably interchangeable within a single deduction graph such as $\Omega$ due to the constraint on their possible positions in $\Omega$. That means their implied existence is shared between all of them, but their representational meaning (metaphorical qualia?) is distinguishable by the deduction graph that they're in, or for that matter, different deduction graphs cannot substitute one another. Thus unique data structures may be created from equivalents (and there's no constraint saying those data structures can't grow and expand over time, or step between deduction graphs dynamically to thereby reuse nothing-subscripts to represent 0s and 1s, rather than the simpler formulation in the Turing machine above).</br>
&ensp;&ensp;&bull;&nbsp; Heck, if I really wanted to construct a specific "Cosmogenesis Myth", I could define a stochastic recursive function through this mechanism, via a deduction graph that points stochastically from the original nothing to a binary bit (or some other encoding), that infinitely points to the original nothing of a next deduction graph which does the same thing, inductively bootstrapping a universe.
...
And that path makes the most sense because we have seen that, otherwise, assuming math combined with information patterns as the fundamental all, then the above deduction is the truth of cosmogenesis, something I consider to be a bigger absurdity than the alternative: the existence of a matter that is neither math nor information pattern — perhaps some kind of singularizing qualia.
