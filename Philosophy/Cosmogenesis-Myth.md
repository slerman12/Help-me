...
> One more critique is that, why go this roundabout way and not just represent the representational universe with some hypothetical decimal, e.g. $\pi$, that perhaps contains within it an encoding of everything? Well, the number line is a bigger assumption than the one I'm making, and requires specific definitions assumed as true. I have made no further assumptions than deduction itself.

That being said, there is now the critique of how I obtain the subscripts of the nothings if there is no a priori number line (though one can of course be defined, e.g., with decimal integers and operators such as $\lt, \gt$). The subscripts don't need to be numbers. They can be any existing, distinguishing variable. For example, a hidden definition in the definition of definition is the existence of that definition. So an arbitrary definiton, such as of numbers, can be constructed as a placeholder for any such such subscript variables, without actually assuming the existence of those numbers or operators. See [Afterward ](#Afterward) for how this hack can be exploited further.
